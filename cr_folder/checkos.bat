...
FINDSTR /C:"%device%" %plinkout1% >nul && (
  plink.exe -ssh tester@%pcIP% -pw fortinet "ls -w 1 /opt/Burnin-tester/ProjectCode/Config/config/Product/FortiGate/image/%device%/TrafficCfg" > %plinkout1%
) || (
...
echo/&echo/
echo %device% OS Versions supported on Rack-%rackname% :
type %xmlout%
del/Q %xmlout%
del/Q %plinkout1%
